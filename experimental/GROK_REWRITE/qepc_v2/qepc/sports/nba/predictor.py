...
class GamePredictor:
    ...
    def prepare(self, cutoff_date: str = None, verbose: bool = True):
        ...
        cutoff_date : str, optional
            For backtesting - only use data before this date
        verbose : bool
            Print progress
        """
        if verbose:
        ...
        # Don't use live ratings when backtesting (they're current, not historical)
        use_live = cutoff_date is None
        
        self._strengths = self.strength_calc.calculate_all_strengths(
            cutoff_date=cutoff_date,
            use_live_ratings=use_live,
            verbose=verbose
        )
