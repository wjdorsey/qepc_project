...
def build_player_points_expectations(
    ...
    *,
    ...
    # minutes expected (prior) — **reactive** blend of last + short + long history (leakage-safe)
    df["minutes_last"] = df.groupby("player_id", sort=False)["minutes_actual"].shift(1)
    df["minutes_mean3"] = _prior_group_rolling_mean(df, "player_id", "minutes_actual", window=3)
    df["minutes_mean10"] = _prior_group_rolling_mean(df, "player_id", "minutes_actual", window=10)

    # Weighting rationale:
    # - last game captures role changes quickly
    # - 3-game mean smooths single-game noise
    # - 10-game mean anchors long-run role
    df["minutes_expected"] = (
        0.60 * df["minutes_last"]
        + 0.30 * df["minutes_mean3"]
        + 0.10 * df["minutes_mean10"]
    )
    # minutes expected (prior)
    df["minutes_expected"] = _prior_group_rolling_mean(
        df, "player_id", "minutes_actual", window=int(cfg.recent_window)
    )
    ...
    # Keep both raw + calibrated outputs:
    df["predicted_points_raw"] = pd.to_numeric(df["predicted_points"], errors="coerce")

    # Optional: apply default affine calibration from a repo-relative JSON.
    # This keeps behavior consistent across machines as long as the JSON is committed.
    cal_path = root / "qepc" / "nba" / "calibration" / "player_points_affine.json"
    if cal_path.exists():
        try:
            ...
            df["predicted_points"] = intercept + slope * df["predicted_points_raw"]
            if clip_nonneg:
                df["predicted_points"] = df["predicted_points"].clip(lower=0.0)

            df["points_affine_slope"] = slope
            df["points_affine_intercept"] = intercept
        except Exception:
            # If calibration file is malformed, fall back to raw predictions silently.
