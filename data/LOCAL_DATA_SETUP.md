# QEPC Local Data Setup

Some large CSVs are not stored in the repo (or appear as Git LFS pointers).
To run QEPC with full local data (no API), create:
    ...
    qepc_project/
      data/
        raw/
          ...
          TeamStatistics.csv
          Games.csv              (optional)
          LeagueSchedule24_25.csv (optional)
          LeagueSchedule25_26.csv (optional)

You can either:

1. Copy your existing CSVs into `data/raw/` with exactly these names, OR
2. Run the notebooks in `notebooks/Data Upgrades/` to generate them from `nba_api`.

Without these files, QEPC will still run using `nba_api` fallbacks, but
some backtests and diagnostics will show warnings.

----------------------------------------------------------------------------------

Code for scanning and exporting sample csvs:
